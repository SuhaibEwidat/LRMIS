...
from LRMIS_BACKEND.models.application_model import application_model


def create_application_service(data, repository, idempotency_key=None):
    ...
    last =   collection.find_one(sort=[("application_id", -1)])
    last_number = 1
    ...
    app = collection.find_one({"_id": result.inserted_id})
    app["_id"] = str(app["_id"])
    return app